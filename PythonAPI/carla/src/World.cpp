...
#define SPAWN_ACTOR_WITHOUT_GIL(fn) +[]( \
  ...
  class_<cc::World>("World", no_init)
	...
	.def("get_gauges_transform", CALL_RETURNING_LIST(cc::World, GetGaugesTransform))
	.def("line_trace_single", CALL_WITHOUT_GIL_1(cc::World, LineTraceSingle, std::string), (arg("json_params")))
	.def("line_trace_multiple", CALL_WITHOUT_GIL_1(cc::World, LineTraceMultiple, std::string), (arg("json_params")))
	.def("line_trace_single_from_player_camera", CALL_WITHOUT_GIL_1(cc::World, LineTraceSingleFromPlayerCamera, std::string), (arg("json_params")))
    // Geometry drawer (JSON string APIs)
    .def("draw_point", CALL_WITHOUT_GIL_1(cc::World, DrawGeometryPoint, std::string), (arg("json")))
    .def("draw_line", CALL_WITHOUT_GIL_1(cc::World, DrawGeometryLine, std::string), (arg("json")))
    .def("draw_cube", CALL_WITHOUT_GIL_1(cc::World, DrawGeometryCube, std::string), (arg("json")))
    .def("remove_draw_object", CALL_WITHOUT_GIL_1(cc::World, RemoveGeometryObject, std::string), (arg("json")))
    .def("clear_draw_objects", CALL_WITHOUT_GIL_1(cc::World, ClearGeometryObjects, std::string), (arg("json")))
    .def("get_spectator", CONST_CALL_WITHOUT_GIL(cc::World, GetSpectator))
    .def("create_robot", CALL_WITHOUT_GIL_1(cc::World, CreateRobot, std::string), (arg("json")))
    .def("destroy_robot", CALL_WITHOUT_GIL_1(cc::World, DestroyRobot, carla::ActorId), (arg("robot_id")))
